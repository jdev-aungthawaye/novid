 ...
 */
public class DefaultIconGenerator<T extends XClusterItem> implements IconGenerator<T> {
    ...
    @NonNull
    public BitmapDescriptor getClusterIcon(@NonNull XCluster<T> XCluster) {
        int clusterBucket = getClusterIconBucket(XCluster);
    ...
    private int getClusterIconBucket(@NonNull XCluster<T> XCluster) {
        int itemCount = XCluster.getItems().size();
